...
final class RiverExplorerGame extends GameView {
    ...
    public boolean onTouchEvent(MotionEvent e) {
        if (e.getActionMasked() == MotionEvent.ACTION_UP) {
            ...
            } else if (x >= albumBtnL - dp(8f) && x <= albumBtnR + dp(8f) && y >= albumBtnT - dp(8f) && y <= albumBtnB + dp(8f)) {
                albumOpen = true;
                albumPage = 0;
    ...
    private void updateTraffic(float dt) {
        ...
        for (Traffic t : traffic) {
            ...
            double z = t.pos - along;
            if (!t.passed && z < 0) {
                t.passed = true;
                passedToday++;
                passedLifetime++;
                showToast("OVERTOOK A " + TRAFFIC_NAME[t.kind] + "  ·  " + passedToday + " THIS ROW", ACCENT);
        ...
        if (!started || rapidsNear || active >= 2 || sessionSeconds < nextSpawnAt || boat.value() < 1f) {
            return;
        }
        for (Traffic t : traffic) {
            if (t.active) {
            ...
            t.pos = along + (t.kind == SCULL ? 90 : LOOK * 0.6);
    ...
    protected void render(Canvas c, float dt) {
        ...
        for (int layer = 0; layer < 2; layer++) {
            paint.setColor(blend(lit(layer == 0 ? 0xFF6E86A0 : 0xFF4E6B5C), fogColour, 0.2f + mist * 0.5f));
            path.reset();
        ...
        for (float y = h; y > horizon + 2; y -= dp(3f)) {
            ...
            paint.setColor(world(grassBand ? 0xFF4F8A3C : 0xFF487F36, fog * 0.7f));
            ...
            if (white) {
                ...
                water = world(ripple ? 0xFF2F6E93 : 0xFF2A6488, fog * 0.6f);
            }
            ...
            paint.setColor(world(0xFF8A7A55, fog * 0.7f));
        ...
        for (long n = lastTree; n >= firstTree; n--) {
            ...
            paint.setColor(world(((hsh >>> 20) & 1) == 0 ? 0xFF2E5E2A : 0xFF3C7033, fog * 0.7f));
    ...
    private void drawTraffic(Canvas c, float w, float horizon, float focal, float yaw) {
        ...
        for (Traffic t : traffic) {
            ...
            float z = (float) (t.pos - along);
            if (z < 4f || z > LOOK) {
                continue;
            }
            if (!t.passed && (nearest == null || z < nearest.pos - along)) {
                nearest = t;
            }
        }
    ...
    private void drawOneBoat(Canvas c, float w, float horizon, float focal, float yaw, Traffic t, float z, boolean tagged) {
        int zi = (int) z;
        float u = focal / z;
        float x = w / 2f + (offsets[zi] + t.lane - lateral - yaw * z) * u;
        float y = horizon + CAM_H * u;
        float fog = Math.min(1f, z / (LOOK * (1f - 0.55f * mist)));
    ...
    private void drawHud(Canvas c, float w, float h, float speed, double forkDist, char choice) {
        ...
        label(c, lightName(), dp(18f), dp(70f), 10f, light < 0.5f ? 0xFFB9C4FF : 0xFFF2C39A, Paint.Align.LEFT);
